...
def format_time(elapsed):
    elapsd_rounded = int(round((elapsed)))
...
def get_score(test_path, co_score_path, true_labels , predictions, counter, topic, result_path):
	...
	if counter == 14:
		...
		tsv_file = "AraCWA(TxtGen).tsv"
		excel_file = "AraCWA(TxtGen).xlsx"
...
def main(args):
	print(" Start AraCWA(TxtGen) Experiment")
	...
	Gen_smpl_set = ["CT20-AR-01_smpl_200_Gen" ,"CT20-AR-02_smpl_200_Gen", "CT20-AR-05_smpl_200_Gen", "CT20-AR-08_smpl_200_Gen", "CT20-AR-10_smpl_200_Gen",
    "CT20-AR-12_smpl_200_Gen", "CT20-AR-14_smpl_200_Gen", "CT20-AR-19_smpl_200_Gen", "CT20-AR-23_smpl_200_Gen", "CT20-AR-27_smpl_200_Gen", "CT20-AR-30_smpl_200_Gen",
    "Covid-19_smpl_200_Gen", "CT21-AR-01_smpl_200_Gen", "CT21-AR-02_smpl_200_Gen"]
	...
	for i in range(0,14):
		...
		Gen_samples_path = os.path.join(args.Gen_samples_data_path,Gen_smpl_set[i]+suffix)
		test_path = os.path.join(args.test_data_path,test_sets[i]+suffix)
		co_score_suffix= '_scores_AraCWA(TxtGen).tsv'
		co_score_path = os.path.join(args.pred_data_path, topics[i] + co_score_suffix)
		pred_suffix= '_pred_AraCWA(TxtGen).tsv'
		...
		Gen_samples_data = pd.read_csv(Gen_samples_path, delimiter = "\t" ,encoding='utf-8')
		...
		Gen_samples_data.drop("tweet_text", axis=1, inplace=True)
		Gen_samples_data = Gen_samples_data[["topic_id" , "tweet_id" , "Gen_text", "claim_worthiness" , "claim"]] 
		Gen_samples_data.rename(columns = {'Gen_text':'tweet_text'}, inplace = True)
		train_data = pd.concat([train_data, Gen_samples_data])
...
if __name__ == '__main__':
	...
	parser.add_argument('--ori_samples_data_path', default="./samples_200/", help='path of original samples')
	parser.add_argument('--Gen_samples_data_path', default="./Gen_200/", help='path of Gen samples')
	parser.add_argument('--test_data_path', default="./new_gold/", help=' path of test set')
